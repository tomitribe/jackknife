...
// Verify that jackknife:index creates a manifest for a test-scope dependency
// (IndexMojo uses ResolutionScope.TEST, so test deps should be indexed)
...
assert utilManifest != null : 'Should have manifest for tomitribe-util (test scope should be indexed via ResolutionScope.TEST)'

// -- USAGE.md generated --
def usageMd = new File(jackknife, 'USAGE.md')
assert usageMd.exists() : 'USAGE.md should be generated'

println "single-index-test-dependency verified:"
