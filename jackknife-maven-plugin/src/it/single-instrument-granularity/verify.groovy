...
// Verify that className+methodName instrumentation (all overloads of join)
// produces debug output for both Join.join(String, Collection) and
// Join.join(String, Object...) when the tests exercise both.
...
// -- Both overloads should produce output --
// The test calls joinCollection (Collection overload) and joinVarargs (Object... overload).
// Count the number of call events for join — should be at least 2.
def callCount = (logContent =~ /"event":"call".*?"method":"join"/).count
assert callCount >= 2 : "Expected at least 2 call events for join, got ${callCount}"
...
println "single-instrument-granularity verified:"
println "  Build succeeded: yes"
println "  Tests ran: yes"
println "  Handler registered: yes"
println "  Debug output for Join.join: yes"
println "  Both overloads produced output: yes (${callCount} call events)"
println "  Patched jar with handlers.properties: yes"
