...
public class IndexMojo extends AbstractMojo {
    ...
    private void writeUsage(final File jackknife) throws IOException {
        ...
        try (final PrintWriter out = new PrintWriter(new FileWriter(usageFile))) {
            ...
            out.println();
            out.println("### Step 1: Check source/ first");
            out.println();
            out.println("Before invoking Maven, check if the class has already been decompiled:");
            out.println("```");
            out.println("Glob .jackknife/source/**/<ClassName>.java");
            out.println("```");
            out.println("Or if you know the groupId:");
            out.println("```");
            out.println("Read .jackknife/source/<groupId>/<artifact>-<version>/com/example/MyClass.java");
            out.println("```");
            out.println();
            out.println("If the file exists, read it directly. No Maven invocation needed.");
            out.println();
            out.println("### Step 2: Decompile if not cached");
            out.println();
            out.println("If the source file does not exist, decompile the jar:");
            out.println("```");
            out.println("mvn jackknife:decompile -Dclass=com.example.MyClass");
            out.println("```");
            out.println();
            out.println("This decompiles the ENTIRE jar containing that class (~3-5s).");
            out.println("Every class in the jar is written as an individual .java file.");
            out.println("All subsequent reads from that jar are direct file access — no Maven.");
            out.println();
