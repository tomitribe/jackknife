#!/usr/bin/env bash
#
# Publish the Jackknife documentation site to GitHub Pages.
#
...
# Check for uncommitted changes in site/
if ! git diff --quiet -- site/; then
    red "You have uncommitted changes in site/. Commit or stash them first."
    exit 1
fi
...
if $DRY_RUN; then
    echo ""
    bold "Dry run — skipping publish. Output is in site/public/"
    exit 0
fi
